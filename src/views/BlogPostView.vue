...
async function loadPost(slug: string) {
  ...
  try {
    ...
    useSeo({
      ...
      url: `https://eventshoot.nl/blog/${result.slug.current}`,
    })
  } finally {
    loading.value = false
  }
  ...
  <main>
    ...
    <div v-else-if="notFound" class="post-state section">
      <div class="container">
        ...
        <RouterLink to="/blog" class="btn btn--outline" style="margin-top:1.5rem">← Terug naar nieuws</RouterLink>
      </div>
    </div>
    ...
    <div v-else-if="post" class="post">
      ...
      <div class="container post__body">
        ...
        <RouterLink to="/blog" class="post__back">← Terug naar nieuws</RouterLink>
      </div>
    </div>
  </main>
</template>
...
:deep(.post__content h3) {
  font-size: 1.2rem;
  font-weight: 700;
  margin: 2rem 0 0.5rem;
}

:deep(.post__content strong) {
  color: var(--color-accent);
  font-weight: 700;
}

:deep(.post__content img) {
  width: 100%;
